  ...
  gv.AgGrid.Host = class AgGridHost {
      ...
      if (gridDiv) {
        ...
        this.gridApi.addEventListener("modelUpdated", function () {
          ...
          if (countEl && this.gridApi) {
            countEl.textContent =
              gvT("grid.records_label", "Records:") + " " + this.gridApi.getDisplayedRowCount();
          }
        }.bind(this));
    ...
    loadState(options) {
      ...
      const pageState = Object.keys(urlPageState).length
        ? Object.assign({}, state.pageState || {}, urlPageState)
        : state.pageState || {};
      ...
      if (
        options.reloadInfinite &&
        this.gridApi.getModel &&
        this.gridApi.getModel().getType() === "infinite"
      ) {
    ...
    _initColSettings() {
      ...
      const preferencesUrl =
        (window.GridView && window.GridView.preferencesUrl) || "";
