...
class GridManagerActionMarkupTests(SimpleTestCase):
    def test_gear_button_uses_data_attributes(self) -> None:
        template = Template(
            "{% load django_grid_view %}{% render_django_grid_view_gear 'doctor-26488' %}"
        )
        html = template.render(Context())
        self.assertIn('data-cm-col-action="toggle"', html)
        self.assertIn('data-cm-grid-id="doctor-26488"', html)
        self.assertNotIn("onclick=", html)
        self.assertNotIn("Manager", html)

    def test_modal_uses_data_attributes(self) -> None:
        template = Template(
            "{% load django_grid_view %}{% render_django_grid_view_modal 'doctor-26488' %}"
        )
        html = template.render(Context())
    ...
    def test_search_bar_uses_data_attributes(self) -> None:
        template = Template(
            "{% load django_grid_view %}{% render_django_grid_view_search 'doctor-26488' %}"
        )
        html = template.render(Context())
